...
function safefind
{
	...
	find $dashh $dashl $dashp "${DIRS[@]}" -nowarn -not \( -name '.Private' -or -name '.git' -prune -or -name '.svn' -prune -or -name '.venv' -prune -or -name '.virtualenv' -prune -or -name 'node_modules' \) \( "$@" \)
}
