...
@test "$FUT: requires at least one argument" {
	run ${EXE}
	assert_failure
	args=
	for i in `seq 5`; do
...
@test "$FUT: finds existing checkouts" {
	run ${EXE} repo1
	assert_success
...
@test "$FUT: finds existing folder in checkout" {
...
@test "$FUT: disambiguates by branch, then folder" {
...
@test "$FUT: prints all ambiguous choices" {
	run $EXE repo2
	assert_success
