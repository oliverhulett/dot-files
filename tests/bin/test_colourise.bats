...
@test "$FUT: uses first line to detect logging format.  Optiver C++" {
	run "$EXE" <<-EOF
...
@test "$FUT: uses first line to detect logging format.  Optiver Python" {
	run "$EXE" <<-EOF
...
@test "$FUT: will colourise a given file" {
...
@test "$FUT: will colourise multiple given files" {
...
@test "$FUT: first arg can be a pattern" {
...
@test "$FUT: if first arg is a pattern then still accepts file args" {
	scoped_mktemp TESTFILE --suffix=.log
	cat >"${TESTFILE}" <<-EOF
		Line should be colourised
		10:11:12.123456789 [module1] Default regex should pick colour1
