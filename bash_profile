...
if [ -d "${HOME}/bin" ] ; then
	PATH="$HOME/bin:$(echo $PATH | sed -re 's!'"$HOME"'/bin/?:?!!g')"
fi
# set PATH so it includes user's private bin if it exists
if [ -d "$HOME/sbin" ]; then
	PATH="$HOME/sbin:$(echo $PATH | sed -re 's!'"$HOME"'/sbin/?:?!!g')"
fi
# set PATH so it includes sbin if it exists
PATH="$(echo $PATH | sed -re 's!:?/usr/local/sbin/?!!g' | sed -re 's!:?/usr/sbin/?!!g' | sed -re 's!:?/sbin/?!!g'):/usr/local/sbin:/usr/sbin:/sbin"
